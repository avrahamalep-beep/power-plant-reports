...
async function init() {
  if (USE_PG) {
    ...
    `);
    await pool.query(
      `ALTER TABLE reports ADD COLUMN IF NOT EXISTS reporter_id TEXT NOT NULL DEFAULT ''`
    );
    await pool.query(
      `ALTER TABLE reports ADD COLUMN IF NOT EXISTS reporter_name TEXT NOT NULL DEFAULT ''`
    );
...
async function listReports() {
  if (USE_PG) {
    const { rows } = await pool.query(
      `SELECT id, kks, location, description, sent_to, custom_recipient, new_issue_note,
...
async function createReport(body) {
  ...
  if (USE_PG) {
    await pool.query(
      `INSERT INTO reports (id, kks, location, description, sent_to, custom_recipient, new_issue_note,
                            reporter_id, reporter_name, created_at, updated_at)
...
async function updateReport(id, body) {
  if (USE_PG) {
    ...
    const reporterId = body.reporterId !== undefined ? body.reporterId : existing.reporterId;
    const reporterName = body.reporterName !== undefined ? body.reporterName : existing.reporterName;
    const now = new Date().toISOString();
    await pool.query(
      `UPDATE reports SET kks = $2, location = $3, description = $4, sent_to = $5::jsonb,
        custom_recipient = $6, new_issue_note = $7, reporter_id = $8, reporter_name = $9,
        updated_at = $10::timestamptz WHERE id = $1`,
      [
        id,
        kks,
        loc,
  ...
  if (body.reporterId !== undefined) r.reporterId = body.reporterId;
  if (body.reporterName !== undefined) r.reporterName = body.reporterName;
...
async function deleteAttachment(reportId, body) {
  const now = new Date().toISOString();
  if (USE_PG) {
    const attachmentId = (body && body.attachmentId) || '';
    if (!attachmentId) return null;
    const exists = await pool.query('SELECT 1 FROM reports WHERE id = $1', [reportId]);
    if (!exists.rowCount) return null;
    await pool.query('DELETE FROM report_files WHERE id = $1 AND report_id = $2', [attachmentId, reportId]);
    await pool.query('UPDATE reports SET updated_at = $2::timestamptz WHERE id = $1', [reportId, now]);
    return getReport(reportId);
  }
  ...
  const r = reports.find(x => x.id === reportId);
  if (!r) return null;
  const targetPath = ((body && body.path) || '').trim();
  if (!targetPath) return r;
  const idx = (r.attachments || []).findIndex(a => a && a.path === targetPath);
  if (idx !== -1) {
    const removed = r.attachments[idx];
    r.attachments.splice(idx, 1);
    const rel = String((removed && removed.path) || '').replace(/^\//, '');
    if (rel.startsWith('uploads/')) {
      ...
      if (fs.existsSync(filePath)) {
        try {
          ...
          console.error('Could not remove attachment file:', e);
        }
      }
    }
    r.updatedAt = now;
...
/** For outbound email: full file buffers for every attachment on this report. */
async function getAttachmentBuffersForReport(reportId) {
  if (USE_PG) {
    const { rows } = await pool.query(
      `SELECT name, mimetype, data FROM report_files WHERE report_id = $1 ORDER BY created_at ASC`,
      [reportId]
    );
    return rows.map(r => ({
      filename: r.name || 'attachment',
      contentType: r.mimetype || 'application/octet-stream',
      buffer: Buffer.isBuffer(r.data) ? r.data : Buffer.from(r.data)
    }));
  }
  const report = await getReport(reportId);
  if (!report || !Array.isArray(report.attachments)) return [];
  const out = [];
